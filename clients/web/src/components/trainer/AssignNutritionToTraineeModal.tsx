...
import { useTrainerNutritionPlans } from '@/hooks/useNutritionPlans';
import { assignNutritionPlan, unassignNutritionPlan } from '@/services/trainer';
import { toast } from 'sonner';

interface AssignNutritionToTraineeModalProps {
  isOpen: boolean;
  onClose: () => void;
  traineeId: string;
  traineeName: string;
...
const AssignNutritionToTraineeModal = ({
  isOpen,
  onClose,
  traineeId,
  traineeName,
  currentPlanId,
  onSuccess,
}: AssignNutritionToTraineeModalProps) => {
  const { t } = useTranslation();
  const { data: plans = [], isLoading } = useTrainerNutritionPlans();
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      if (isUnassigning) {
        const { error } = await unassignNutritionPlan(
          traineeId,
          selectedPlanId,
        );
        if (error) {
          ...
          toast.success(`Nutrition plan unassigned from ${traineeName}`);
          onSuccess?.();
          onClose();
        }
      } else {
        const { error } = await assignNutritionPlan({
          nutritionPlanId: selectedPlanId,
          traineeId: traineeId,
        ...
        if (error) {
          ...
          toast.success(
            t('nutrition.assignedSuccess', { trainee: traineeName }) ||
              `Nutrition plan assigned to ${traineeName}`,
          );
          onSuccess?.();
          onClose();
        }
                  ...
                  >
                    ...
                    <div className="flex-grow">
                      ...
                      <p className="font-body text-xs text-muted-foreground">
                        {plan.goal} • {plan.totals?.calories || 0} kcal
                      </p>
                    </div>
                    {isSelected && (
...
export default AssignNutritionToTraineeModal;
